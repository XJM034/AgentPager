...
AgentGrid 把 Codex Desktop 与 Codex CLI 的实时运行态同步到旧 Android 手机。手机端是原生横屏应用，不依赖浏览器，使用低分辨率彩色像素块、常驻硬边光晕、状态音效和振动表达任务状态。

## 已实现

- Codex 开始、思考、工具执行、成功、失败、中断、等待批准、等待回答和离线状态。
- 5×5 彩色像素核心；活跃状态 60 FPS，终态 30 FPS，空闲和离线 12 FPS。
- 开始、成功、失败、等待批准、等待回答和中断的 8-bit 状态音。
- 多任务排序、左右滑动切换、长按固定、双击静音。
