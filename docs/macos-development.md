...
固定标识文件只保存 40 位十六进制证书指纹，不保存私钥，且不进入仓库。配置无效或所选身份无法签名时停止，不静默降级。脚本不创建证书、不导入私钥、不写系统信任，也不自动创建该配置文件；首次启用需要用户批准。更换签名前备份该文件和现有 App；获取 Apple 开发证书使用 Xcode 的证书管理或自动签名流程，登录及双重验证由用户完成，不撤销既有证书来强行通过。

文件钥匙串同时检查应用访问列表和分区权限，不能仅凭相同 Bundle ID 或应用身份规则判断更新后免授权。临时签名、本机自签证书仍使用会随构建变化的 `cdhash:` 分区；Apple 签发的开发证书可提供稳定的团队分区，但仍需真实升级验收。本地自签证书、Apple Development 都不等于 Developer ID 正式分发签名或 Apple 公证，不适用于对外发布结论。签名身份变更必须重新验收配对与 GLM 读取权限。

只读诊断：`swift -suppress-warnings scripts/inspect-macos-keychain-access.swift --expect-stable`。该命令只查询已安装 App 的公开签名信息和两个既有条目的权限元数据，不读取 Key、不修改权限、不触发授权窗口；缺少 Apple 团队签名时退出 1。检查通过只代表签名先决条件具备，不代替真实凭据的跨版本读取验证。
...
- 固定签名后仍需比较两个不同构建的 designated requirement（系统用于识别应用身份的规则），确认代码指纹确实不同，并用真实登录钥匙串验证升级与重启后的读取、额度更新和提示事件。隔离合成条目、相同 Bundle ID 或签名检查通过都不能单独证明授权复用。

隔离签名检查可运行 `python3 scripts/test-macos-keychain-signing.py`（需要 OpenSSL 3 和当前 Swift 工具链）。它生成一天有效的临时自签证书和合成 Key，测试重启、更新及改签后的静默访问，并读取测试条目的分区 ACL。临时钥匙串缺少分区 ACL 时退出 2，明确报告覆盖缺口，不能声称真实升级授权已通过；有分区 ACL 时，相同自签证书的新二进制应被静默拒绝。结束时删除测试钥匙串和私钥文件，并核对用户搜索列表与默认钥匙串未改变；结果写入被忽略的 `dist/glm-signing-probe-result.json`。该脚本不启用正式本机签名、不导入登录钥匙串、不修改系统信任。早先未检查分区 ACL 的成功结论只覆盖旧式测试数据库，见 [诊断更正](updates/2026-08-31-macos-keychain-partition-signing.md)。
